...
public partial class AppropriationPaginatedListHandler
{
    ...
    public class Handler(AppDbContext dbContext, IMapperAdapter mapper) : HandlerBase<AppDbContext, Request, Response>(dbContext, mapper)
    {
        public async override Task<Response> Handle(Request request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var paginatedQuery = await _dbContext.Appropriations.SimplePaginatedQuery(request.Query, cancellationToken);
                ...
                var entities = await paginatedQuery.Queryable
                    //.ProjectTo<AppropriationDto>(_mapper.ConfigurationProvider)  /// awh - no support for custom resolvers -- https://docs.automapper.org/en/stable/Queryable-Extensions.html#supported-mapping-options
